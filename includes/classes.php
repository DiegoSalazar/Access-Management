...
class MemSeriesNumberGenerator {
	...
	*/
	...
	function __constructor() {
		...
		if (!$con) die('Error connecting to DB. in '.__FILE__.' Line: '.__LINE__.'<br>MySQL error: '.mysql_error());
		mysql_select_db('deeniesdb');
	}	
	
	public function getCurrentSeries() {
		$sql = "SELECT member_num FROM members WHERE member_num != 'none' ORDER BY id DESC LIMIT 1";
